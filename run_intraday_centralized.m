...
function [deltas, success] = run_intraday_centralized(plan_DA, forecasts_RT, k, H, initial_soc_mg1, initial_soc_mg2, initial_soc_mg3)
% ======================== MODIFICATION END ========================
...
%
% 输入:
%   plan_DA          - 日前调度计划结果的结构体
%   forecasts_RT     - 包含各主体在滚动窗口内超短期预测数据的结构体
%   k                - 当前日内调度的时间点 (1-96)
%   H                - 滚动优化时域的长度
%   initial_soc_mg1  - 时刻k开始时MG1的实际SOC
%   initial_soc_mg2  - 时刻k开始时MG2的实际SOC
%   initial_soc_mg3  - 时刻k开始时MG3的实际SOC
%
% 输出:
%   deltas           - 包含所有调整量优化结果的结构体
%   success          - 求解是否成功的标志 (true/false)

    yalmip('clear');
    T = H; % 优化时域长度

    fprintf('--- 执行集中式日内调度 (k=%d, H=%d) ---\n', k, T);

    %% ======================= 1. 定义权重系数 =======================
    w = struct();
    w.dso_grid   = 0.005; w.dso_dr     = 0.002; w.dso_ses    = 0.003;
    w.mg_gt      = 0.1;   w.mg_bat     = 0.05;  w.mg_dr      = 0.08;
    w.seso_own   = 0.02;  w.violation  = 50;
    ...
    % --- 循环构建每个时间点的约束 ---
    P_batc_actual_mg1 = sdpvar(1, T); P_batd_actual_mg1 = sdpvar(1, T); % Preallocate for auxiliary function output
